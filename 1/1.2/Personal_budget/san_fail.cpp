...
public:
  ...
  explicit BulkLinearUpdater(const BulkMoneyAdder& add)
      : add_(add)
  {}

  explicit BulkLinearUpdater(const BulkTaxApplier& tax)
      : tax_(tax)
  {}
...
private:
  ...
  static unique_ptr<Node> Build(IndexSegment segment) {
    ...
    if (segment.length() > 1) {
      ...
      node->left = Build({segment.left, middle});
      node->right = Build({middle, segment.right});
    }
    return node;
  }
