...
namespace Graph {
  ...
  std::optional<typename Router<Weight>::RouteInfo> Router<Weight>::BuildRoute(VertexId from, VertexId to) const {
    ...
    while (!vertices_by_weight.empty()) {
      ...
      const Weight min_weight = min_vertex_it->first;
      ...
      for (const EdgeId edge_id : graph_.GetIncidentEdges(min_vertex_id)) {
        ...
        if (edge.to >= routes_internal_data_.size()) {
          continue;
        }
        ...
        if (!routes_internal_data_[edge.to] || routes_internal_data_[edge.to]->weight > min_weight + edge.weight) {
          if (routes_internal_data_[edge.to]) {
            vertices_by_weight.erase({routes_internal_data_[edge.to]->weight, edge.to});
          }
          routes_internal_data_[edge.to] = RouteInternalData{.weight = min_weight + edge.weight,
                                                             .prev_edge = edge_id};
    ...
    VertexId current_vertex = to;
    while (true) {
      if (current_vertex >= routes_internal_data_.size()) {
        break;
      }

      const auto& data = routes_internal_data_[current_vertex];
      if (!data || !data->prev_edge) {
        break;
      }

      const EdgeId edge_id = *data->prev_edge;
      edges.push_back(edge_id);
      current_vertex = graph_.GetEdge(edge_id).from;
    }
